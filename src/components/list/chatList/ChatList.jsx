...
import { MdOutlineGroupAdd, MdOutlinePushPin, MdDeleteOutline } from "react-icons/md";
  ...
  const ChatItem = ({ isFirst, isLast, currentChat, onClick }) => {
    ...
    return (
      <div className={`transition cursor-pointer duration-300 ${currentChat?.chatId === chat?.chatId ? 'bg-gray-600' : ' hover:bg-gray-400'}`}>
        ...
        ${isFirst ? 'border-b' : isLast ? '' : 'border-y'} border-gray-700`}
          onClick={onClick}
        >
          {currentChat?.user?.avatar ? (
            <img src={currentChat?.user?.avatar} alt="User Avatar" className="w-10 h-10 object-cover object-top border rounded-full" />
          ) : (
          ...
          <div className='flex items-center justify-between w-full'>
            ...
            <div className="relative">
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-36 bg-gray-700 border border-gray-600 rounded-md shadow-md z-10 text-sm">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  <button
                    ...
                    className="w-full flex items-center rounded-b-md gap-2 text-left px-4 py-2 hover:bg-red-600 text-red-300"
                  >
  ...
  return (
    <div className='flex flex-col flex-1 overflow-hidden px-4 py-3 bg-gray-800 text-white rounded-lg shadow-lg'>
      ...
      <div className="flex-1 overflow-y-auto rounded-md scrollbar-thin scrollbar-thumb-sky-700 scrollbar-track-sky-300">
        {userChats?.length === 0 ? (
          <div className="flex flex-col items-center justify-center flex-1 w-full rounded-md text-gray-400 h-full">
